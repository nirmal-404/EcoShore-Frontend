    ...
    return (
        <div className="container mx-auto px-6 py-12">
            <div className="mb-10">
                ...
                <p className="text-muted-foreground mt-2">Enter collection data for completed beach cleanup events.</p>
            </div>
                        ...
                        events?.map(event => (
                            ...
                            >
                                <CardContent className="p-4 flex items-center justify-between">
                                    <div>
                                        <h3 className="font-bold truncate max-w-[150px]">{event.title}</h3>
                                        <p className="text-xs text-muted-foreground">{event.beach?.name}</p>
                                    </div>
                                    <Badge variant="secondary" className="text-[10px]">{new Date(event.date).toLocaleDateString()}</Badge>
                                </CardContent>
                            </Card>
                        ))
                    )}
                ...
                <div className="lg:col-span-2">
                    {selectedEvent ? (
                                ...
                                <div className="flex justify-between items-start">
                                    <div>
                                        ...
                                        <CardDescription className="flex items-center mt-1">
                                            <MapPin className="w-3 h-3 mr-1" /> {selectedEvent.beach?.name}
                                        </CardDescription>
                                    </div>
                                ...
                                <form onSubmit={handleRecord} className="space-y-6">
                                    <div className="grid md:grid-cols-2 gap-6">
                                        <div className="space-y-2">
                                            <label className="text-sm font-semibold">Waste Type Description</label>
                                            <input
                                                ...
                                                onChange={e => setWasteData({ ...wasteData, wasteType: e.target.value })}
                                                required
                                            />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-sm font-semibold">Waste Category</label>
                                            <select
                                                className="w-full p-3 rounded-xl border bg-white focus:ring-2 focus:ring-primary/10 outline-none"
                                                value={wasteData.category}
                                                onChange={e => setWasteData({ ...wasteData, category: e.target.value })}
                                            >
                                    ...
                                    <div className="p-4 rounded-xl bg-secondary/20 flex gap-3 text-sm text-secondary-foreground items-start">
                                        ...
                                        <p>Submitting this record will automatically mark the event as <strong>Completed</strong>. This action cannot be undone by the collector.</p>
                                    </div>

                                    <div className="flex justify-end gap-3 pt-4 border-t">
                                        <Button variant="ghost" type="button" onClick={() => setSelectedEvent(null)}>Cancel</Button>
                                        <Button type="submit" className="rounded-xl px-10" disabled={recordMutation.isPending}>
                                            {recordMutation.isPending ? 'Saving...' : 'Finalize Record'}
                                        </Button>
                                    </div>
                                </form>
                            </CardContent>
                        ...
                        <div className="h-full min-h-[400px] flex flex-col items-center justify-center text-center p-12 border-2 border-dashed border-border rounded-2xl bg-secondary/5">
                            ...
                            <p className="text-muted-foreground max-w-sm">Please select a cleanup event from the list on the left to start logging waste collection data.</p>
                        </div>
                    )}
                </div>
            </div>
