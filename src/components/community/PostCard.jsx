        ...
        <Card className="rounded-3xl border-border/60 shadow-sm hover:shadow-md transition-shadow overflow-hidden bg-card/50 backdrop-blur-sm">
            <CardHeader className="pb-3 flex flex-row items-start justify-between">
                <div className="flex gap-4 items-center">
                    ...
                    <div>
                        <h3 className="font-bold text-foreground leading-tight">{post.authorId?.name}</h3>
                ...
                {post.mediaUrls && post.mediaUrls.length > 0 && (
                    <div className={`grid gap-2 mb-2 ${post.mediaUrls.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                        {post.mediaUrls.map((img, i) => (
                            <div key={i} className="aspect-video bg-muted rounded-xl overflow-hidden border border-border/50">
            ...
            <CardFooter className="flex flex-col border-t border-border/40 pt-3 pb-3 px-6 bg-secondary/5">
                <div className="flex justify-between w-full">
                    <Button
                        ...
                        size="sm"
                        onClick={handleLike}
                        className={`flex gap-2 rounded-full px-4 transition-colors ${isLiked ? 'text-rose-500 hover:text-rose-600 hover:bg-rose-50' : 'text-muted-foreground hover:text-rose-500'
                            }`}
                    >
                    ...
                    >
                        ...
                        <span className="font-medium">{post.comments?.length || post.commentCount || 0}</span>
                    </Button>
