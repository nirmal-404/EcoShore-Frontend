...
import {
    Plus, X, Loader2, ChevronDown, Search,
    CheckSquare, Square, Users, UserCheck,
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { cn } from '@/lib/utils';

const TYPE_OPTIONS = [
    { value: 'GLOBAL_VOLUNTEER', label: 'Global Volunteer', description: 'Open to all volunteers' },
    { value: 'ORGANIZER_PRIVATE', label: 'Organizer Private', description: 'Organizers only' },
    { value: 'EVENT_GROUP', label: 'Event Group', description: 'Linked to a specific event' },
];
...
function MemberPicker({ selected, onChange }) {
    ...
    const allActive = activeIds.length > 0 && activeIds.every((id) => selected.includes(id));
    ...
    const TABS = [
        ...
        { id: 'organizers', label: 'Organizers', icon: UserCheck, count: organizers.length },
    ];
                ...
                {TABS.map(({ id, label, icon: Icon, count }) => (
                    ...
                    >
                        ...
                        <span className={cn(
                            ...
                            tab === id ? 'bg-primary/10 text-primary' : 'bg-secondary/50 text-muted-foreground'
                        )}>
                            {count}
                        </span>
                    </button>
            ...
            <div className="p-4 space-y-3.5">
                ...
                {createMutation.isError && (
                    <p className="text-xs text-destructive bg-destructive/10 border border-destructive/20 rounded-lg px-3 py-2">
                        {createMutation.error?.response?.data?.message || 'Failed to create group. Try again.'}
                    </p>
                )}
