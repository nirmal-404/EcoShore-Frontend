      ...
      <div className="w-full h-[500px] rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-600 relative z-0">
        <MapContainer center={center} zoom={7} scrollWheelZoom={false} style={{ height: '100%', width: '100%' }}>
          ...
          {beaches?.map((beach) => {
             ...
             else if (beach.currentSeverityLevel === 'MODERATE') color = '#eab308';
               ...
               >
                 <Popup>
                   <div className="font-sans font-medium text-gray-800">
                     ...
                     <p className="text-xs">Severity Level: 
                        <span className="font-bold ml-1" style={{ color }}>{beach.currentSeverityLevel}</span>
                     </p>
                     <p className="text-xs">Score: {beach.currentSeverityScore.toFixed(1)}</p>
                   </div>
                 </Popup>
               </CircleMarker>
             );
